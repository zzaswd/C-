...
public:
	...
	bool SaveImage(const char* filename) { 
		...
		if (!strcmp(".bmp", &filename[strlen(filename) - 4])) { // 일정 문자열에서 .의 주소를 알기 위해서 주소 연산자 사용. 끝에서 4자 .bmp만큼 주소를 shift 시킨 것이다.
			FILE* pFile = NULL;
			fopen_s(&pFile, filename, "wb"); // wb : write byte. 바이트 단위로 쓰겠다. 첫 번째 매개변수가 주소값이 들어가있으므로 그 값을 byte단위로 받아온 것. 포인터면 그 주소를 받아 옴. 
			...
			if (m_nChannels == 1) {		// palete 정보
				for (int l = 0; l < 256; l++) {		
					RGBQUAD GrayPalette = { l,l,l,0 };		//grayscale을 담당.
